...
import tools.code_size
from tools.shellutil import *
import tools.build_utils
...
variants = {
    ...
    "alabs_pythonteam": {"capas": """ALL -TCB -DRM -AllFiles""", "key": "pythonteam"},
    "selfsigned": {
    ...
    "white_choco": {"capas": """ALL -TCB""", "key": "pythonteam"},
    "dark_choco": {"capas": """ALL -TCB""", "key": "rd02"},
    "unsigned_high_capas": {
        "capas": "LocalServices NetworkServices ReadUserData"
        + " WriteUserData UserEnvironment Location"
...
deliverables_name = {
    "30armv5": {
        ...
        "sdk_zip_name": ["Python_SDK_3rdEd.zip", "Python_%s%s_SDK_3rdEd.zip"],
        "run_testapp_sis_name": [
            "run_testapp_3rdEd.SIS",
    ...
    "50armv5": {
        ...
        "sdk_zip_name": ["Python_SDK_5thEd.zip", "Python_%s%s_SDK_5thEd.zip"],
    },
    "31": {"sdk_zip_name": ["Python_SDK_3rdEd.zip", "Python_%s%s_SDK_3rdEdFP1.zip"]},
    "32": {
        "python_dll_sis_name": [
            "Python25_3rdEdFP2.SIS",
            "Python_%s%s_3rdEd_%s.sis",
            "\\newcore\\symbian\\group",
        ],
        "testapp_sis_name": [
            "testapp_3rdEdFP2.SIS",
            "testapp_%s%s_3rdEd_%s.sis",
            "\\ext\\test\\testapp\\group",
            "test",
        ],
        "sdk_zip_name": ["Python_SDK_3rdEdFP2.zip", "Python_%s%s_SDK_3rdEdFP2.zip"],
        "run_testapp_sis_name": [
            "run_testapp_3rdEdFP2.SIS",
            "run_testapp_%s%s_3rdEd_%s.sis",
            "\\ext\\test\\run_testapp\\group",
            "test",
        ],
...
def move_deliverable_to_readybuild_dir(platform, flavor):
    ...
    for deliverable in platform_name.keys():
        ...
        final_sis_name = platform_name[deliverable][1] % (version, version_tag, flavor)
        ...
        shutil.move(final_sis_name, "%s\\%s" % (output_dir, final_sis_name))
    os.chdir(topdir)
...
def create_and_move_device_deliverables(platform, flavor):
    ...
    if not "key" in variants[flavor]:
        ...
        run_cmd(
            "python setup.py bdist_sis --keydir %s --key %s"
    ...
    if flavor == "unsigned_alabs":
        ...
        passphrase = ""
        os.chdir(topdir + deliverables_name[platform]["python_dll_sis_name"][2])
        ...
        run_cmd(
            ...
            + " "
            + final_name
            + " %s %s %s" % (cert, key, passphrase)
        )
        shutil.move(final_name, "%s\\%s" % (python_readybuild_dir, final_name))
        os.chdir(topdir)
...
def do_setupdotpy_configure(flavor, platform):
    ...
    global setup_configure
    setup_configure = (
        "python setup.py configure --sdk %s --version %s \
    ...
    log("""builder.py - Building platform: %s and flavor: %s""" % (platform, flavor))
...
def create_and_move_zip(platforms):
    ...
    def create_sdk_zip(platforms, armv5_pyds=False):
        ...
        shutil.move(
            final_sdk_zip_name, "%s\\%s" % (python_readybuild_dir, final_sdk_zip_name)
        )
...
def init_args(params):
    ...
    parser.add_option(
        ...
        + "%s [default: all the flavors]" % ",".join(variants.keys()),
    )
    parser.add_option(
        "--work-area",
        dest="work_area",
    ...
    parser.add_option("--keydir", dest="key", default="..\\keys", help="Key path")
    parser.add_option(
        "-d",
        "--docs",
        action="store_true",
    ...
    parser.add_option(
        ...
        dest="installer",
        default=False,
        help="Builds and creates a Setup.exe for PYS60 " + "[default: %default]",
    )
    parser.add_option(
        "--create-temp-sdk-zip",
        action="store_true",
        ...
        default=False,
        help="Creates an sdkzip with all armv5 binaries" + "[default: %default]",
    )
...
class BuildInstaller(Thread):
    ...
    def run(self):
        ...
        package_files = {
            ...
            "..\\tools\\py2sis\\ensymble_ui\\images\\": ["python_logo.gif"],
            "..\\tools\\py2sis\\ensymble_ui\\": [
                "ensymble_gui.pyw",
                "ensymble_ui_help.html",
            ],
            topdir
            + "\\tools\\installer\\doc\\": ["Quickguide.html", "python_logo.PNG"],
        }
        ...
        for path in package_files:
            for entry in package_files[path]:
                ...
                if os.path.isdir(entry_path):
                    shutil.copytree(entry_path, os.path.join(package_dir, entry))
                else:
        ...
        for deps in package_dependencies:
            ...
            shutil.copy(deps_path + deps, package_dependencies_dir + "\\" + deps)
        ...
        def generate_setup():
            ...
            installer_build_cmd = (
                'IsCmdBld.exe -p "%s" -r "%s" -c \
                COMP -a "%s"'
                % (ism_path, release_name, project_configuration)
            )
            os.system(installer_build_cmd)
            os.chdir(current_dir)
            setup_file = "PythonForS60_%s_%s_Setup.exe" % (version, version_tag)
            setup_dir = (
                "PythonForS60\\%s\\Beta\\DiskImages\\DISK1" % project_configuration
            )
            os.rename(
                installer_dir + "%s\\setup.exe" % setup_dir,
        ...
        def build_pys60_package():
            ...
            shutil.copytree(
                package_dir, pys60_archive_dir + "\\" + os.path.basename(package_dir)
            )
            shutil.copytree(
                package_dependencies_dir,
                pys60_archive_dir
                + "\\PythonForS60\\"
                + os.path.basename(package_dependencies_dir),
            )
            ...
            create_archive_from_directory(
                topdir + "\\build\\PythonForS60_%s_%s.tar.gz" % (version, version_tag),
                pys60_archive_dir,
                archive_type="tar.gz",
            )
...
def call_ensymble(tmp_dir, ensymble_options, flavour):
    ...
    ensymble_cmd = "python ensymble.py py2sis %s %s" % (tmp_dir, ensymble_options)
    ...
    try:
        ...
        if flavour == "high_capas_pythonteam":
            run_cmd("signsis %s %s %s %s" % (sis_name, sis_name, sign_cert, sign_key))
    except:
...
def create_sdk_zip_31():
    ...
    sdk_zip_name = deliverables_name["32"]["sdk_zip_name"][1] % (version, version_tag)
...
def generate_scriptshell_sis():
    ...
    for flavour in flavours:
        ...
        common_opt = (
            "-v --appname=%s --version=%s --heapsize=%s "
        ...
        if integration_bld:
            sis_name = "PythonScriptShell_%s%s_%s.sis" % (version, version_tag, flavour)
        else:
    ...
    for path in package_files:
        for entry in package_files[path]:
            ...
            if os.path.isdir(entry_path):
                ...
                shutil.copy(entry_path, package_dir)
    create_archive_from_directory(
        topdir + "\\build\\ensymble_%s_%s.zip" % (version, version_tag), package_dir
    )
...
def test_sdk_zip(sdk_version):
    ...
    build_utils.unzip_file_into_dir(topdir + "\\build\\" + sdk_zip_name, "\\")
    cmd_test = "python setup.py test --testset-size 350 --sdk-version %s" % sdk_version
    ...
    try:
        ...
        run_cmd(
            "signsis elemlist.sis %s %s %s" % (elemlist_sis_name, sign_cert, sign_key)
        )
        shutil.copy(
            elemlist_sis_name,
...
def release_build():
    ...
    os.rename(
        topdir + "\\build\\" + sdk_zip_name, topdir + "\\build\\" + final_sdk_zip_name
    )
    ...
    zip_object = zipfile.ZipFile(topdir + "\\build\\" + final_sdk_zip_name, "a")
    zip_object.write(pyd_path + "kf_scriptext.pyd", pyd_path + "kf_scriptext.pyd")
    zip_object.close()
    ...
    test_sdk_zip("_3_2")
    thread_pool = []
    cmd_remote_buid = "test_device_remote --work-area " + python_readybuild_dir
    ...
    if without_src_zip == False:
        # Create src zip
        zipname = "%s\\" % (python_readybuild_dir) + "pys60-%s%s_src.zip" % (
            version,
            version_tag,
        )
...
def main():
    ...
    log(
        "builder.py - Please find the deliverables in directory: %s"
        % python_readybuild_dir
    )
...
def show_configuration():
    log(
        "builder.py : The following values are considered for this build.\n"
        + "Target -----------------> %s \n" % target
        + "Version ----------------> %s \n" % version
        + "Version tag ------------> %s \n" % version_tag
        + "Build profile ----------> %s \n" % build_profile
...
if __name__ == "__main__":
    ...
    temp = time.strftime(
        python_readybuild_dir + "\\test\\pys60_build_log_%Y%m%d_%H%M.log"
    )
    log_file = tee(temp, "w")
