...
SYSTEMINCLUDE    \epoc32\include\python38
SYSTEMINCLUDE    \epoc32\include\stdapis
SYSTEMINCLUDE    \epoc32\include

// libc and euser are always needed when using main() entry point
LIBRARY python38.lib libc.lib euser.lib
...
def pkg_append(modulelist):
    ...
    pkgfile = open("..\\group\\python38.pkg.in.template", "r")
    pkgfile_new = open("..\\group\\python38.pkg.in", "w")
    for line in pkgfile:
        pkgfile_new.write(line)
        if ";PYDs" in line:
            break
    for module in modulelist:
        print("Adding :" + module + ".pyd to python38.pkg.in")
        pkgfile_new.write(
...
def genconfigdotc(modulelist):
    ...
    configdotc.write(
        "\n /* Refer the file 'modules.cfg' for the list of " + "built-in modules */ \n"
    )
...
def pythonmmp_append(file_list):
    ...
    python38 mmp file"""
    print(" Adding files :", file_list, " to python38.mmp.in")
    pythonmmp = open("..\\group\\python38.mmp.in.template", "r")
    pythonmmp_new = open("..\\group\\python38.mmp.in", "w")
...
def generate_pymodules_zip(base_py_module_paths, base_py_module_names, zip_file):
...
def generate_module_repo():
    ...
    module_repo_dir = os.path.abspath(
        "..\\..\\..\\tools\\py2sis\\ensymble\\module-repo\\"
    )
    standard_modules_dir = os.path.join(module_repo_dir, "standard-modules")
    device_binaries_path = os.path.abspath(
        "\\epoc32\\release\\%s\\%s" % (DEVICE_PLATFORM, DEVICE_BUILD)
    )
    ...
    for src_path, dest_path in zip(
        cfg_data["ext_modules"], cfg_data["ext_module_names"]
    ):
        src_path = os.path.abspath(src_path)
        dest_path = os.path.join(dev_module_dir, dest_path)
        copy_file(src_path, dest_path)

    if os.path.exists(module_repo_dir) and os.path.exists(dev_module_dir):
        content = []
        open(os.path.join(dev_module_dir, "module_search_path.cfg"), "w+").write(
            str(content)
        )

    for mod in cfg_data["std_repo_pyds"]:
        if not os.path.exists(os.path.join(standard_modules_dir, mod)):
            copy_file(
                os.path.join(device_binaries_path, mod),
                os.path.join(standard_modules_dir, mod),
            )

    for i in range(len(cfg_data["ext_mod_cfg_paths"])):
        abs_cfg_path_src = os.path.join(
            os.path.abspath(cfg_data["ext_mod_cfg_paths"][i]), "module_config.cfg"
        )
        abs_cfg_path_dest = os.path.join(
            os.path.abspath(
                "..\\..\\..\\tools\\py2sis\\ensymble\\module-repo\\dev-modules"
            ),
            cfg_data["ext_mod_cfg_names"][i],
            "module_config.cfg",
        )
    ...
    for i in range(len(cfg_data["ext_pyds"])):
        if not os.path.exists(
            ...
            copy_file(
                os.path.abspath(cfg_data["ext_pyds"][i]),
                os.path.join(dev_module_dir, cfg_data["ext_pyd_names"][i]),
            )
...
def readconfig_and_generate():
        ...
        if line and line[0] != "#":
            if module_type == "PYD":
                ...
                if module_data[1].endswith(".py"):
                    ...
                    dep_map[
                        module_data[1].replace(".py", "").replace("\\", ".")
                    ] = mod_map
                ...
                try:
                    ...
                    ext_mod_type = "repo"
                ext_modules.append(
                    line.split(":")[0] + os.path.basename(line.split(":")[1])
                )

                if ext_mod_type != "base":
                    repo_py_module_paths.append(line.split(":")[0])
                    repo_py_module_names.append(os.path.basename(line.split(":")[1]))
                    mod_repo_excluded_list.append(
                        line.split(":")[0] + os.path.basename(line.split(":")[1])
                    )
                else:
                    base_py_module_paths.append(line.split(":")[0])
                    base_py_module_names.append(os.path.basename(line.split(":")[1]))
                    mod_repo_base_excluded_list.append(
                        line.split(":")[0] + os.path.basename(line.split(":")[1])
                    )
                ...
                try:
                    ...
                    ext_pyd_type = "repo"
                ext_pyd_file = os.path.basename(os.path.splitext(line.split(":")[1])[0])
    ...
    for i in range(len(repo_py_module_paths)):
        std_base_pys.append(repo_py_module_paths[i] + repo_py_module_names[i])
    ...
    for module in modulename_in_list:
        modulename_in.write(os.path.dirname(os.getcwd()) + "\\group\\" + module + "\n")
    for files in fileutil.all_files(os.path.dirname(os.getcwd()), "*.template"):
...
def get_py_pyd_files(arg, dirname, files):
    ...
    if (
        ...
        and dirname not in dirname_list
    ):
    ...
    for name in names:
        ...
        if name.endswith(".c") or name.endswith(".h") and not name == "main.c":
            ...
            for line in fp:
                ...
                standard_deps.sort()
                mod_map["deps"] = []
                if pyd_name in dep_map and dep_map[pyd_name]["type"] != "excluded":
...
if __name__ == "__main__" or __name__ == "__builtin__":
    ...
    if not MOD_REPO:
        ...
        generate_pymodules_zip(
            cfg_data["base_py_module_paths"],
            cfg_data["base_py_module_names"],
            base_modules_zip,
        )
        generate_pymodules_zip(
            cfg_data["repo_py_module_paths"],
        ...
        extn_mod_repo = cfg_data["base_py_module_names"][
            (len(cfg_data["std_repo_pys"])) :
        ]
        extn_mod_base = cfg_data["repo_py_module_names"][
            (len(cfg_data["std_base_pys"])) :
        ]
        ...
        repo_mod_dest_path = (
            cfg_data["base_py_module_names"] + cfg_data["repo_py_module_names"]
        )
        generate_module_repo()

        module_repo_path = os.path.abspath(
            "..\\..\\..\\tools\\py2sis\\ensymble\\module-repo"
        )
        std_module_search_path = os.path.join(module_repo_path, "standard-modules")
        ...
        for path in ["Modules", "Objects", "Python", "PC"]:
            os.path.walk(os.path.abspath("..\\..\\" + path), find_std_pyd_deps, None)

        # Write the dependency map in a sorted order
        fp = open(os.path.join(std_module_search_path, "module_dependency.cfg"), "wb")
        fp.write("{")
        ...
        # Check if line below is really needed
