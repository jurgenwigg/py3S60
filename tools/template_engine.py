...
def outfilename_from_infilename(infilename):
    if not is_templatefile(infilename):
        raise ValueError(
            "expected file name with suffix "
            + TEMPLATEFILE_SUFFIX
            + ", got "
            + infilename
        )
    ...
    def process_text(text):
        ...
        while 1:
            ...
            if m:
                ...
                pos = m.start()
                paren_level = 0
                for m in macro_delimiter_re.finditer(
                    text, pos
                ):  # find a single whole macro expression
                    delim = m.group(1)
                    if delim == "${{":
                        paren_level += 1
                    elif delim == "}}":
                    ...
                    if paren_level < 0:
                        raise MacroEvaluationError(
                            "Mismatched parens in macro %s" % text[pos : m.end()]
                        )
